        ...
        for(i = 0; i < h; i++)
        {
            for(j = 0; j < w; j++)
            {
                ...
                case ImageFilterType::GaussainLowPass:
                    ...
                    break;
                case ImageFilterType::GaussainHighPass:
                    filter[c*pixel_num + i * w + j] = 1-exp(-area1 / (2*r*r));
                    break;
...
{
    QImage filteredSpectrumImage = src;
    int w = src.width();
    int h = src.height();
    int n = w*h;
    ...
    float* filter = new float[3*n];
    int r = 5;
    generateFilter(w, h, r, IdealLowPass, filter);
    imageFilterFFT2D(temp, w, h, filter, filteredSpectrumImage, dst);
...
void IdealHighPassFilter(QImage src, QImage &dst)
{
    ...
    float* filter = new float[3*n];
    int r = 5;
    generateFilter(w, h, r, IdealHighPass, filter);
    imageFilterFFT2D(temp, w, h, filter, filteredSpectrumImage, dst);
...
void GaussainLowPassFilter(QImage src, QImage &dst)
{
    ...
    float* filter = new float[3*n];
    int r = 50;
    generateFilter(w, h, r, GaussainLowPass, filter);
    imageFilterFFT2D(temp, w, h, filter, filteredSpectrumImage, dst);
...
void GaussainHighPassFilter(QImage src, QImage &dst)
{
    ...
    float* filter = new float[3*n];
    int r = 5;
    generateFilter(w, h, r, GaussainHighPass, filter);
    imageFilterFFT2D(temp, w, h, filter, filteredSpectrumImage, dst);
...
void ButterworthHighPassFilter(QImage src, QImage &dst)
{
    ...
    float* filter = new float[3*n];
    int r = 5;
    generateFilter(w, h, r, ButterworthHighPass, filter);
    imageFilterFFT2D(temp, w, h, filter, filteredSpectrumImage, dst);
...
void ButterworthLowPassFilter(QImage src, QImage &dst)
{
    ...
    float* filter = new float[3*n];
    int r = 5;
    generateFilter(w, h, r, ButterworthLowPass, filter);
    imageFilterFFT2D(temp, w, h, filter, filteredSpectrumImage, dst);
