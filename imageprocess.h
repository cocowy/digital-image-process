#ifndef IMAGEPROCESS_H
#define IMAGEPROCESS_H

#include <QString>
#include <QFileDialog>
#include <QLabel>
#include <QDebug>
#include <QGraphicsScene>
#include <QGraphicsView>
#include "opencv2/opencv.hpp"
#include "opencv2/features2d.hpp"

#include <vector>
#include <iostream>
#include <QImage>
#include <QColor>
#include <complex>
#include "transform.h"

using namespace cv;

enum ImageChannel
{
    Y=0,
    R,
    G,
    B
};

enum ColorMap
{
    Jet = 0,
    Parula,
    Hot
};

QImage calculateHistogram(QImage &image, ImageChannel channel);
QImage calculateNegative(QImage &image, ImageChannel channel);
QImage convertToPseudoColor(QImage &image, ColorMap map);
QImage equalizeHistogramProc(QImage &image);
QImage equalizeHistogramProc1(QImage &image);
QImage AverageFilter(QImage &image);
QImage GaussianFilter(QImage &image);
QImage MedianFilter(QImage &image);

void paddingZeros(const float *r, const float *g, const float *b,
                   const int width, const int height,
                   const int half_pad_width, const int half_pad_height,
                   float *nr, float *ng, float *nb);
void splitImageChannel(QImage &image, uchar *r, uchar *g, uchar *b);
void splitImageChannel(QImage &image, uchar *rgb);
void splitImageChannel(QImage &image, float *r, float *g, float *b);
void splitImageChannel(QImage &image, float *rgb);
void concatenateImageChannel(float *r, float *g, float *b, int w, int h, QImage &image);
void concatenateImageChannel(float *rgb, int w, int h, QImage &image);
void concatenateImageChannel(uchar *r, uchar *g, uchar *b, int w, int h, QImage &image);
void concatenateImageChannel(uchar *rgb, int w, int h, QImage &image);
void rgb2ycrcb(uchar *r, uchar *g, uchar *b, int size, float *y, float *cr, float *cb);
void ycrcb2rgb(float *y, float *cr, float *cb, int size, uchar *r, uchar *g, uchar *b);
void calculate_integral_image(float *image, int width, int height, float *integral_image);
void calculate_integral_image_power(float *image, int width, int height, float *integral_image);
__inline float box_integral(float *integral_image, int width, int height, int c1, int c2, int r1, int r2);
void adaptiveContrastEnhancement(QImage &src_image, float *rgb, float *rgb_ii, float *rgb_ii_power, int max_window_size,
                                 int half_window_size, float alpha, float max_cg, QImage &dst_image);
void filter(uchar *src, int image_width, int image_height, int coefficient,
            float *kernel, int kernel_width, int kernel_height, uchar *dst);
void medianf_ilter(uchar *src, int image_width, int image_height, int coefficient,
            float *kernel, int kernel_width, int kernel_height, uchar *dst);

void edgefilter(float *src, int image_width, int image_height, float *kernel1, float *kernel2, int kernel_width, int kernel_height, float* dst);
void logfilter(float *src, int image_width, int image_height, float *kernel, int kernel_width, int kernel_height, float* dst);
void EdgeFilterRobert(QImage src, QImage &dst);
void EdgeFilterSobel(QImage src, QImage &dst);
void EdgeFilterLOG(QImage src, QImage &dst);
void EdgeFilterPerwitt(QImage src, QImage &dst);

void SIFTMatching(QImage src, QImage &dst);
void ORBMatching(QImage src, QImage &dst);
void FastMatching(QImage src, QImage &dst);
void AddNoiseSaltPepper(QImage src, QImage &dst);
void AddNoiseGuassian(QImage src, QImage &dst);

QImage mat2qim(const cv::Mat& mat);
void qimage_to_mat(const QImage& image, cv::OutputArray out);

float Sort(float a1[],int n);

const uchar hot_table[]={
    3, 0, 0,
    5, 0, 0,
    8, 0, 0,
    11, 0, 0,
    13, 0, 0,
    16, 0, 0,
    19, 0, 0,
    21, 0, 0,
    24, 0, 0,
    27, 0, 0,
    29, 0, 0,
    32, 0, 0,
    35, 0, 0,
    37, 0, 0,
    40, 0, 0,
    43, 0, 0,
    45, 0, 0,
    48, 0, 0,
    51, 0, 0,
    53, 0, 0,
    56, 0, 0,
    59, 0, 0,
    61, 0, 0,
    64, 0, 0,
    67, 0, 0,
    69, 0, 0,
    72, 0, 0,
    75, 0, 0,
    77, 0, 0,
    80, 0, 0,
    83, 0, 0,
    85, 0, 0,
    88, 0, 0,
    91, 0, 0,
    93, 0, 0,
    96, 0, 0,
    99, 0, 0,
    101, 0, 0,
    104, 0, 0,
    107, 0, 0,
    109, 0, 0,
    112, 0, 0,
    115, 0, 0,
    117, 0, 0,
    120, 0, 0,
    123, 0, 0,
    125, 0, 0,
    128, 0, 0,
    131, 0, 0,
    133, 0, 0,
    136, 0, 0,
    139, 0, 0,
    141, 0, 0,
    144, 0, 0,
    147, 0, 0,
    149, 0, 0,
    152, 0, 0,
    155, 0, 0,
    157, 0, 0,
    160, 0, 0,
    163, 0, 0,
    165, 0, 0,
    168, 0, 0,
    171, 0, 0,
    173, 0, 0,
    176, 0, 0,
    179, 0, 0,
    181, 0, 0,
    184, 0, 0,
    187, 0, 0,
    189, 0, 0,
    192, 0, 0,
    195, 0, 0,
    197, 0, 0,
    200, 0, 0,
    203, 0, 0,
    205, 0, 0,
    208, 0, 0,
    211, 0, 0,
    213, 0, 0,
    216, 0, 0,
    219, 0, 0,
    221, 0, 0,
    224, 0, 0,
    227, 0, 0,
    229, 0, 0,
    232, 0, 0,
    235, 0, 0,
    237, 0, 0,
    240, 0, 0,
    243, 0, 0,
    245, 0, 0,
    248, 0, 0,
    251, 0, 0,
    253, 0, 0,
    255, 0, 0,
    255, 3, 0,
    255, 5, 0,
    255, 8, 0,
    255, 11, 0,
    255, 13, 0,
    255, 16, 0,
    255, 19, 0,
    255, 21, 0,
    255, 24, 0,
    255, 27, 0,
    255, 29, 0,
    255, 32, 0,
    255, 35, 0,
    255, 37, 0,
    255, 40, 0,
    255, 43, 0,
    255, 45, 0,
    255, 48, 0,
    255, 51, 0,
    255, 53, 0,
    255, 56, 0,
    255, 59, 0,
    255, 61, 0,
    255, 64, 0,
    255, 67, 0,
    255, 69, 0,
    255, 72, 0,
    255, 75, 0,
    255, 77, 0,
    255, 80, 0,
    255, 83, 0,
    255, 85, 0,
    255, 88, 0,
    255, 91, 0,
    255, 93, 0,
    255, 96, 0,
    255, 99, 0,
    255, 101, 0,
    255, 104, 0,
    255, 107, 0,
    255, 109, 0,
    255, 112, 0,
    255, 115, 0,
    255, 117, 0,
    255, 120, 0,
    255, 123, 0,
    255, 125, 0,
    255, 128, 0,
    255, 131, 0,
    255, 133, 0,
    255, 136, 0,
    255, 139, 0,
    255, 141, 0,
    255, 144, 0,
    255, 147, 0,
    255, 149, 0,
    255, 152, 0,
    255, 155, 0,
    255, 157, 0,
    255, 160, 0,
    255, 163, 0,
    255, 165, 0,
    255, 168, 0,
    255, 171, 0,
    255, 173, 0,
    255, 176, 0,
    255, 179, 0,
    255, 181, 0,
    255, 184, 0,
    255, 187, 0,
    255, 189, 0,
    255, 192, 0,
    255, 195, 0,
    255, 197, 0,
    255, 200, 0,
    255, 203, 0,
    255, 205, 0,
    255, 208, 0,
    255, 211, 0,
    255, 213, 0,
    255, 216, 0,
    255, 219, 0,
    255, 221, 0,
    255, 224, 0,
    255, 227, 0,
    255, 229, 0,
    255, 232, 0,
    255, 235, 0,
    255, 237, 0,
    255, 240, 0,
    255, 243, 0,
    255, 245, 0,
    255, 248, 0,
    255, 251, 0,
    255, 253, 0,
    255, 255, 0,
    255, 255, 4,
    255, 255, 8,
    255, 255, 12,
    255, 255, 16,
    255, 255, 20,
    255, 255, 24,
    255, 255, 28,
    255, 255, 32,
    255, 255, 36,
    255, 255, 40,
    255, 255, 44,
    255, 255, 48,
    255, 255, 52,
    255, 255, 56,
    255, 255, 60,
    255, 255, 64,
    255, 255, 68,
    255, 255, 72,
    255, 255, 76,
    255, 255, 80,
    255, 255, 84,
    255, 255, 88,
    255, 255, 92,
    255, 255, 96,
    255, 255, 100,
    255, 255, 104,
    255, 255, 108,
    255, 255, 112,
    255, 255, 116,
    255, 255, 120,
    255, 255, 124,
    255, 255, 128,
    255, 255, 132,
    255, 255, 136,
    255, 255, 140,
    255, 255, 144,
    255, 255, 148,
    255, 255, 152,
    255, 255, 156,
    255, 255, 160,
    255, 255, 164,
    255, 255, 168,
    255, 255, 172,
    255, 255, 176,
    255, 255, 180,
    255, 255, 184,
    255, 255, 188,
    255, 255, 192,
    255, 255, 196,
    255, 255, 200,
    255, 255, 204,
    255, 255, 208,
    255, 255, 212,
    255, 255, 216,
    255, 255, 220,
    255, 255, 224,
    255, 255, 228,
    255, 255, 232,
    255, 255, 236,
    255, 255, 240,
    255, 255, 244,
    255, 255, 248,
    255, 255, 252,
    255, 255, 255
};

const uchar jet_table[] = {
    0, 0, 131,
    0, 0, 135,
    0, 0, 139,
    0, 0, 143,
    0, 0, 147,
    0, 0, 151,
    0, 0, 155,
    0, 0, 159,
    0, 0, 163,
    0, 0, 167,
    0, 0, 171,
    0, 0, 175,
    0, 0, 179,
    0, 0, 183,
    0, 0, 187,
    0, 0, 191,
    0, 0, 195,
    0, 0, 199,
    0, 0, 203,
    0, 0, 207,
    0, 0, 211,
    0, 0, 215,
    0, 0, 219,
    0, 0, 223,
    0, 0, 227,
    0, 0, 231,
    0, 0, 235,
    0, 0, 239,
    0, 0, 243,
    0, 0, 247,
    0, 0, 251,
    0, 0, 255,
    0, 4, 255,
    0, 8, 255,
    0, 12, 255,
    0, 16, 255,
    0, 20, 255,
    0, 24, 255,
    0, 28, 255,
    0, 32, 255,
    0, 36, 255,
    0, 40, 255,
    0, 44, 255,
    0, 48, 255,
    0, 52, 255,
    0, 56, 255,
    0, 60, 255,
    0, 64, 255,
    0, 68, 255,
    0, 72, 255,
    0, 76, 255,
    0, 80, 255,
    0, 84, 255,
    0, 88, 255,
    0, 92, 255,
    0, 96, 255,
    0, 100, 255,
    0, 104, 255,
    0, 108, 255,
    0, 112, 255,
    0, 116, 255,
    0, 120, 255,
    0, 124, 255,
    0, 128, 255,
    0, 131, 255,
    0, 135, 255,
    0, 139, 255,
    0, 143, 255,
    0, 147, 255,
    0, 151, 255,
    0, 155, 255,
    0, 159, 255,
    0, 163, 255,
    0, 167, 255,
    0, 171, 255,
    0, 175, 255,
    0, 179, 255,
    0, 183, 255,
    0, 187, 255,
    0, 191, 255,
    0, 195, 255,
    0, 199, 255,
    0, 203, 255,
    0, 207, 255,
    0, 211, 255,
    0, 215, 255,
    0, 219, 255,
    0, 223, 255,
    0, 227, 255,
    0, 231, 255,
    0, 235, 255,
    0, 239, 255,
    0, 243, 255,
    0, 247, 255,
    0, 251, 255,
    0, 255, 255,
    4, 255, 251,
    8, 255, 247,
    12, 255, 243,
    16, 255, 239,
    20, 255, 235,
    24, 255, 231,
    28, 255, 227,
    32, 255, 223,
    36, 255, 219,
    40, 255, 215,
    44, 255, 211,
    48, 255, 207,
    52, 255, 203,
    56, 255, 199,
    60, 255, 195,
    64, 255, 191,
    68, 255, 187,
    72, 255, 183,
    76, 255, 179,
    80, 255, 175,
    84, 255, 171,
    88, 255, 167,
    92, 255, 163,
    96, 255, 159,
    100, 255, 155,
    104, 255, 151,
    108, 255, 147,
    112, 255, 143,
    116, 255, 139,
    120, 255, 135,
    124, 255, 131,
    128, 255, 128,
    131, 255, 124,
    135, 255, 120,
    139, 255, 116,
    143, 255, 112,
    147, 255, 108,
    151, 255, 104,
    155, 255, 100,
    159, 255, 96,
    163, 255, 92,
    167, 255, 88,
    171, 255, 84,
    175, 255, 80,
    179, 255, 76,
    183, 255, 72,
    187, 255, 68,
    191, 255, 64,
    195, 255, 60,
    199, 255, 56,
    203, 255, 52,
    207, 255, 48,
    211, 255, 44,
    215, 255, 40,
    219, 255, 36,
    223, 255, 32,
    227, 255, 28,
    231, 255, 24,
    235, 255, 20,
    239, 255, 16,
    243, 255, 12,
    247, 255, 8,
    251, 255, 4,
    255, 255, 0,
    255, 251, 0,
    255, 247, 0,
    255, 243, 0,
    255, 239, 0,
    255, 235, 0,
    255, 231, 0,
    255, 227, 0,
    255, 223, 0,
    255, 219, 0,
    255, 215, 0,
    255, 211, 0,
    255, 207, 0,
    255, 203, 0,
    255, 199, 0,
    255, 195, 0,
    255, 191, 0,
    255, 187, 0,
    255, 183, 0,
    255, 179, 0,
    255, 175, 0,
    255, 171, 0,
    255, 167, 0,
    255, 163, 0,
    255, 159, 0,
    255, 155, 0,
    255, 151, 0,
    255, 147, 0,
    255, 143, 0,
    255, 139, 0,
    255, 135, 0,
    255, 131, 0,
    255, 128, 0,
    255, 124, 0,
    255, 120, 0,
    255, 116, 0,
    255, 112, 0,
    255, 108, 0,
    255, 104, 0,
    255, 100, 0,
    255, 96, 0,
    255, 92, 0,
    255, 88, 0,
    255, 84, 0,
    255, 80, 0,
    255, 76, 0,
    255, 72, 0,
    255, 68, 0,
    255, 64, 0,
    255, 60, 0,
    255, 56, 0,
    255, 52, 0,
    255, 48, 0,
    255, 44, 0,
    255, 40, 0,
    255, 36, 0,
    255, 32, 0,
    255, 28, 0,
    255, 24, 0,
    255, 20, 0,
    255, 16, 0,
    255, 12, 0,
    255, 8, 0,
    255, 4, 0,
    255, 0, 0,
    251, 0, 0,
    247, 0, 0,
    243, 0, 0,
    239, 0, 0,
    235, 0, 0,
    231, 0, 0,
    227, 0, 0,
    223, 0, 0,
    219, 0, 0,
    215, 0, 0,
    211, 0, 0,
    207, 0, 0,
    203, 0, 0,
    199, 0, 0,
    195, 0, 0,
    191, 0, 0,
    187, 0, 0,
    183, 0, 0,
    179, 0, 0,
    175, 0, 0,
    171, 0, 0,
    167, 0, 0,
    163, 0, 0,
    159, 0, 0,
    155, 0, 0,
    151, 0, 0,
    147, 0, 0,
    143, 0, 0,
    139, 0, 0,
    135, 0, 0,
    131, 0, 0,
    128, 0, 0
};

// 256*3
const uchar parula_table[] = {
    62, 38, 168,
    62, 39, 172,
    63, 40, 175,
    63, 41, 178,
    64, 42, 180,
    64, 43, 183,
    65, 44, 186,
    65, 45, 189,
    66, 46, 191,
    66, 47, 194,
    67, 48, 197,
    67, 49, 200,
    67, 50, 202,
    68, 51, 205,
    68, 52, 208,
    69, 53, 210,
    69, 55, 213,
    69, 56, 215,
    70, 57, 217,
    70, 58, 220,
    70, 59, 222,
    70, 61, 224,
    71, 62, 225,
    71, 63, 227,
    71, 65, 229,
    71, 66, 230,
    71, 68, 232,
    71, 69, 233,
    71, 70, 235,
    72, 72, 236,
    72, 73, 237,
    72, 75, 238,
    72, 76, 240,
    72, 78, 241,
    72, 79, 242,
    72, 80, 243,
    72, 82, 244,
    72, 83, 245,
    72, 84, 246,
    71, 86, 247,
    71, 87, 247,
    71, 89, 248,
    71, 90, 249,
    71, 91, 250,
    71, 93, 250,
    70, 94, 251,
    70, 96, 251,
    70, 97, 252,
    69, 98, 252,
    69, 100, 253,
    68, 101, 253,
    67, 103, 253,
    67, 104, 254,
    66, 106, 254,
    65, 107, 254,
    64, 109, 254,
    63, 110, 255,
    62, 112, 255,
    60, 113, 255,
    59, 115, 255,
    57, 116, 255,
    56, 118, 254,
    54, 119, 254,
    53, 121, 253,
    51, 122, 253,
    50, 124, 252,
    49, 125, 252,
    48, 127, 251,
    47, 128, 250,
    47, 130, 250,
    46, 131, 249,
    46, 132, 248,
    46, 134, 248,
    46, 135, 247,
    45, 136, 246,
    45, 138, 245,
    45, 139, 244,
    45, 140, 243,
    45, 142, 242,
    44, 143, 241,
    44, 144, 240,
    43, 145, 239,
    42, 147, 238,
    41, 148, 237,
    40, 149, 236,
    39, 151, 235,
    39, 152, 234,
    38, 153, 233,
    38, 154, 232,
    37, 155, 232,
    37, 156, 231,
    36, 158, 230,
    36, 159, 229,
    35, 160, 229,
    35, 161, 228,
    34, 162, 228,
    33, 163, 227,
    32, 165, 227,
    31, 166, 226,
    30, 167, 225,
    29, 168, 225,
    29, 169, 224,
    28, 170, 223,
    27, 171, 222,
    26, 172, 221,
    25, 173, 220,
    23, 174, 218,
    22, 175, 217,
    20, 176, 216,
    18, 177, 214,
    16, 178, 213,
    14, 179, 212,
    11, 179, 210,
    8, 180, 209,
    6, 181, 207,
    4, 182, 206,
    2, 183, 204,
    1, 183, 202,
    0, 184, 201,
    0, 185, 199,
    0, 186, 198,
    1, 186, 196,
    2, 187, 194,
    4, 187, 193,
    6, 188, 191,
    9, 189, 189,
    13, 189, 188,
    16, 190, 186,
    20, 190, 184,
    23, 191, 182,
    26, 192, 181,
    29, 192, 179,
    32, 193, 177,
    35, 193, 175,
    37, 194, 174,
    39, 194, 172,
    41, 195, 170,
    43, 195, 168,
    44, 196, 166,
    46, 196, 165,
    47, 197, 163,
    49, 197, 161,
    50, 198, 159,
    51, 199, 157,
    53, 199, 155,
    54, 200, 153,
    56, 200, 150,
    57, 201, 148,
    59, 201, 146,
    61, 202, 144,
    64, 202, 141,
    66, 202, 139,
    69, 203, 137,
    72, 203, 134,
    75, 203, 132,
    78, 204, 129,
    81, 204, 127,
    84, 204, 124,
    87, 204, 122,
    90, 204, 119,
    94, 205, 116,
    97, 205, 114,
    100, 205, 111,
    103, 205, 108,
    107, 205, 105,
    110, 205, 102,
    114, 205, 100,
    118, 204, 97,
    121, 204, 94,
    125, 204, 91,
    129, 204, 89,
    132, 204, 86,
    136, 203, 83,
    139, 203, 81,
    143, 203, 78,
    147, 202, 75,
    150, 202, 72,
    154, 201, 70,
    157, 201, 67,
    161, 200, 64,
    164, 200, 62,
    167, 199, 59,
    171, 199, 57,
    174, 198, 55,
    178, 198, 53,
    181, 197, 51,
    184, 196, 49,
    187, 196, 47,
    190, 195, 45,
    194, 195, 44,
    197, 194, 42,
    200, 193, 41,
    203, 193, 40,
    206, 192, 39,
    208, 191, 39,
    211, 191, 39,
    214, 190, 39,
    217, 190, 40,
    219, 189, 40,
    222, 188, 41,
    225, 188, 42,
    227, 188, 43,
    230, 187, 45,
    232, 187, 46,
    234, 186, 48,
    236, 186, 50,
    239, 186, 53,
    241, 186, 55,
    243, 186, 57,
    245, 186, 59,
    247, 186, 61,
    249, 186, 62,
    251, 187, 62,
    252, 188, 62,
    254, 189, 61,
    254, 190, 60,
    254, 192, 59,
    254, 193, 58,
    254, 194, 57,
    254, 196, 56,
    254, 197, 55,
    254, 199, 53,
    254, 200, 52,
    254, 202, 51,
    253, 203, 50,
    253, 205, 49,
    253, 206, 49,
    252, 208, 48,
    251, 210, 47,
    251, 211, 46,
    250, 213, 46,
    249, 214, 45,
    249, 216, 44,
    248, 217, 43,
    247, 219, 42,
    247, 221, 42,
    246, 222, 41,
    246, 224, 40,
    245, 225, 40,
    245, 227, 39,
    245, 229, 38,
    245, 230, 38,
    245, 232, 37,
    245, 233, 36,
    245, 235, 35,
    245, 236, 34,
    245, 238, 33,
    246, 239, 32,
    246, 241, 31,
    246, 242, 30,
    247, 244, 28,
    247, 245, 27,
    248, 247, 26,
    248, 248, 24,
    249, 249, 22,
    249, 251, 21

};

#endif // IMAGEPROCESS_H
